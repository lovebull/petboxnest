    ...
    <div className="flex flex-col gap-y-3">
      <Container className="p-0">
        <div className="flex flex-col gap-4 border-b px-6 py-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <Heading level="h1">结账错误日志</Heading>
            <Text className="text-ui-fg-subtle" size="small">
              Checkout error logs · 查看脱敏的依赖读取错误并记录处理结果。
            </Text>
          </div>
          <div className="flex flex-wrap gap-2">
            <Stat label="全部 / Total" value={list.data?.summary.total} />
            <Stat label="待处理 / Open" value={list.data?.summary.open} />
            <Stat label="已解决 / Resolved" value={list.data?.summary.resolved} />
            ...
            <Table>
              ...
              <Table.Body>
                {rows.map((record) => (
                  ...
                  >
                    <Table.Cell className="font-mono">{record.error_id}</Table.Cell>
        ...
        <div className="flex flex-col gap-3 border-t px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
          <Text size="small">
            第 {page} 页，共 {list.data?.page_count || 1} 页 · {list.data?.count || 0} 条
          </Text>
          <div className="flex items-center gap-2">
            ...
            >
              <Select.Trigger className="w-24"><Select.Value /></Select.Trigger>
              <Select.Content>
                {[20, 50, 100].map((size) => (
                  <Select.Item key={size} value={String(size)}>{size} 条</Select.Item>
                ))}
              </Select.Content>
            </Select>
            <Button size="small" variant="secondary" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>上一页</Button>
            <Button size="small" variant="secondary" disabled={page >= (list.data?.page_count || 1)} onClick={() => setPage((value) => value + 1)}>下一页</Button>
          </div>
        </div>
      </Container>
      ...
      <ErrorDrawer
        ...
        onUpdate={(resolution_status, admin_note) => {
          if (activeId) update.mutate({ id: activeId, resolution_status, admin_note })
        }}
...
function FilterSelect({ value, onChange, placeholder, options }: { value: string; onChange: (value: string) => void; placeholder: string; options: string[][] }) {
  return (
    <Select value={value} onValueChange={onChange}>
      <Select.Trigger className="lg:w-52"><Select.Value /></Select.Trigger>
      <Select.Content>
        <Select.Item value="all">{placeholder}</Select.Item>
        {options.map(([optionValue, label]) => <Select.Item key={optionValue} value={optionValue}>{label}</Select.Item>)}
      </Select.Content>
    </Select>
  )
}

function ErrorDrawer({ record, open, loading, onOpenChange, onUpdate, updating }: { record: CheckoutError | null; open: boolean; loading: boolean; onOpenChange: (open: boolean) => void; onUpdate: (status: ResolutionStatus, note: string | null) => void; updating: boolean }) {
  const [note, setNote] = useState("")
  useEffect(() => setNote(record?.admin_note || ""), [record?.admin_note])
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <Drawer.Content>
        <Drawer.Header><Drawer.Title>错误日志详情 / Error details</Drawer.Title></Drawer.Header>
        <Drawer.Body className="flex flex-col gap-5 overflow-y-auto p-6">
          {loading && <Text className="text-ui-fg-subtle">正在加载…</Text>}
          {record && (
            <>
              <div>
                <Heading level="h2" className="font-mono">{record.error_id}</Heading>
                <Text size="small" className="text-ui-fg-subtle">{new Date(record.occurred_at).toLocaleString()}</Text>
              </div>
              <div className="grid grid-cols-2 gap-3 rounded-md bg-ui-bg-subtle p-4">
                <Detail label="资源" value={resourceLabels[record.resource]} />
                <Detail label="状态码" value={record.status_code?.toString() || "未提供"} />
                <Detail label="错误代码" value={record.code} />
                <Detail label="可重试" value={record.retryable ? "是" : "否"} />
                <Detail label="购物车哈希" value={record.cart_id_hash || "未提供"} />
                <Detail label="Region ID" value={record.region_id || "未提供"} />
                <Detail label="国家" value={record.country_code?.toUpperCase() || "未提供"} />
                <Detail label="来源" value={record.source} />
              </div>
              <div className="flex flex-col gap-2">
                <Text weight="plus">管理员备注 / Admin note</Text>
                <Textarea value={note} onChange={(event) => setNote(event.target.value)} maxLength={2000} rows={6} placeholder="记录排查原因或处理结果" />
              </div>
              <div className="flex flex-wrap gap-2">
                <Button disabled={updating} onClick={() => onUpdate("resolved", note.trim() || null)}>标记已解决</Button>
                <Button disabled={updating} variant="secondary" onClick={() => onUpdate("open", note.trim() || null)}>恢复待处理</Button>
                <Button disabled={updating} variant="secondary" onClick={() => onUpdate("ignored", note.trim() || null)}>忽略</Button>
              </div>
              {record.resolved_at && <Text size="xsmall" className="text-ui-fg-subtle">最后处理：{new Date(record.resolved_at).toLocaleString()}</Text>}
            </>
          )}
        </Drawer.Body>
      </Drawer.Content>
...
function Detail({ label, value }: { label: string; value: string }) {
  return <div><Text size="xsmall" className="text-ui-fg-subtle">{label}</Text><Text size="small" className="break-all">{value}</Text></div>
}
