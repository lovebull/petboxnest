  ...
  statuses: z
    ...
    .transform((value) =>
      value
        .split(",")
        .filter(Boolean)
        .map((status) => ReviewStatusSchema.parse(status)),
    )
    .optional(),
  ratings: z
    .string()
    .transform((value) =>
      value
        .split(",")
        .filter(Boolean)
        .map((rating) => z.coerce.number().int().min(1).max(5).parse(rating)),
    )
    .optional(),
...
const ReviewReplySchema = z.strictObject({
  content: z.string().trim().min(2).max(2000),
});

const AfterSalesTypeSchema = z.enum([
  "cancel",
  "return",
  "exchange",
  "damaged_claim",
...
const CreateAfterSalesTypeSchema = z.enum([
  "cancel",
  "return",
  "damaged_claim",
  "lost_claim",
...
const AutomationTokenSchema = z.strictObject({
...
export default defineMiddlewares({
  routes: [
    ...
    },
    {
      ...
      method: "POST",
      middlewares: [
        validateAndTransformBody(CreateGuestAfterSalesRequestSchema),
      ],
    },
    {
    ...
    },
    {
      ...
      method: "POST",
      middlewares: [validateAndTransformBody(UpdateCashbackSettingsSchema)],
    },
    {
      matcher: "/admin/cashback/entries",
      method: "GET",
      middlewares: [validateAndTransformQuery(GetCashbackEntriesSchema, {})],
    },
    {
    ...
    },
    {
      ...
      middlewares: [
        validateAndTransformQuery(
          ReviewListSchema.pick({ page: true, limit: true }),
          {},
        ),
      ],
    },
