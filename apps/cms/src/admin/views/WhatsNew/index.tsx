...
const updates = [
  {
    ...
    items: [
      "🔎 新增 Storefront 商品搜索表单组件，支持 Catalog、移动菜单和预留导航三种展示样式，并通过 q 查询参数提交搜索。",
      "🧭 Store、Category 和 Collection 页面接入搜索参数，页面标题、当前浏览卡片和空状态会根据搜索关键词动态展示。",
      "🧠 商品搜索支持匹配商品标题、副标题、描述、handle、集合、分类、商品类型、SKU、规格选项、标签和 metadata 文本。",
      "📱 移动侧边菜单新增商品搜索入口，搜索后自动跳转到当前国家代码下的 /store 并关闭菜单。",
      "🎛️ 目录筛选面板新增 Search 区块，支持保留已有 pet、price、availability、option filters，并可单独清除搜索词。",
      "📦 商品列表读取根据排序、搜索和自定义筛选智能选择 Medusa API 分页或全量批量读取，减少不必要的全目录遍历。",
      "🔢 价格排序、搜索和自定义筛选改为全量筛选后再分页，保证结果数量、页码和当前页商品一致。",
    ],
    fixes: [
      "🐛 修复商品目录没有文本搜索入口，客户无法按商品名、SKU、分类或关键词快速查找商品的问题。",
      "🐛 修复搜索和筛选变化后仍保留旧 page 参数，可能跳到无结果页的问题。",
      "🐛 修复价格排序和自定义筛选只基于单页商品计算，导致跨页排序、计数和分页不准确的问题。",
      "🐛 修复商品列表没有请求 categories 与 variant sku 字段，搜索无法覆盖分类信息和 SKU 的问题。",
      "🐛 修复选项筛选值可能重复写入请求，造成 Store API 查询参数冗余的问题。",
      "🐛 修复目录页标题在搜索状态下仍显示普通 Store/Category/Collection 文案，客户难以确认当前搜索范围的问题。",
      "🐛 修复移动菜单缺少商品搜索入口，移动端客户需要先进入商品页才能开始搜索的问题。",
    ],
  },
  {
    version: "v1.18.0",
  ...
  },
  {
    ...
    items: [
      "🔌 Backend 环境模板默认端口从 8030 统一调整为 7020，并同步更新 ADMIN_CORS、AUTH_CORS、MEDUSA_BACKEND_URL 和文件服务地址。",
      "🧩 CMS 环境示例默认端口从 8020 统一调整为 7010，内部 MEDUSA_BACKEND_URL 默认指向 127.0.0.1:7020。",
      "🛍️ Storefront 环境模板和域名部署示例同步使用 Medusa 7020 与 Payload CMS 7010，保持前后台服务默认端口一致。",
      "🧭 CMS Medusa 产品代理接口默认后端地址改为 127.0.0.1:7020，未配置环境变量时仍能访问当前 Backend。",
      "🌐 Storefront SDK 配置和 middleware 的 Medusa 默认地址改为 7020，避免区域检测与 Store API 请求继续落到旧端口。",
      "📝 Whats New 历史端口说明同步校正为当前 7000、7010、7020 端口体系，减少后续复制配置时误读。",
    ],
    fixes: [
      "🐛 修复 Backend、CMS、Storefront 环境模板仍混用 8020/8030 旧端口，复制配置后服务间请求可能连错端口的问题。",
      "🐛 修复 CMS 商品选择接口未配置 MEDUSA_BACKEND_URL 时默认请求 8030，导致 Payload Admin 读取 Medusa 商品失败的问题。",
      "🐛 修复 Storefront SDK 默认后端地址仍为 8030，缺少环境变量时 Store API 请求无法命中当前 Medusa Backend 的问题。",
      "🐛 修复 Storefront middleware 默认区域检测继续使用 8030，可能导致国家代码重定向或地区 Cookie 初始化失败的问题。",
      "🐛 修复 Storefront 环境模板中 Payload 默认地址仍为 8020，产品增强、文章和 CMS 内容读取可能指向旧端口的问题。",
      "🐛 修复 Whats New 历史描述中的端口号与当前配置不一致，后台更新记录可能误导部署和排查的问题。",
    ],
  },
  {
    version: "v1.17.1",
  ...
  },
  {
    ...
    fixes: [
      "🐛 修复 Storefront 脚本仍运行于 7000，而项目约定和其他服务使用 7000，导致本地访问地址不一致的问题。",
      "🐛 修复 Backend 管理端清理 Storefront 缓存时默认请求 7000，未配置显式地址时刷新失败的问题。",
      "🐛 修复 Payload 文章保存或删除后默认向 7000 发送 Revalidation 请求，文章缓存无法及时更新的问题。",
      "🐛 修复 Medusa Store 与 Auth CORS 默认来源残留 7000，使用 PUBLIC_HOST:7000 访问时可能被跨域策略拦截的问题。",
      "🐛 修复环境模板和域名部署示例继续生成 7000 地址，复制配置后服务间回调指向错误端口的问题。",
  ...
  },
  {
    ...
    fixes: [
      ...
      "🐛 修复 README、本地 CORS 与环境示例仍混用 7000、7020 和旧 CMS 端口，导致本地访问来源配置不一致的问题。",
  ...
  },
  {
    ...
    fixes: [
      ...
      "🐛 修复 Payload/Storefront 默认读取 7010 端口、项目指南记录 7000/9010 端口，和当前 Petboxnest 本地服务端口不一致的问题。",
  ...
  },
  {
    ...
    fixes: [
      "修复 Payload 服务默认地址，统一使用 127.0.0.1:7010，减少 localhost 在服务端环境解析不一致的问题。",
